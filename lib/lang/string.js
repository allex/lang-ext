...
exports.startsWith = function(s, starts, position) {
  s = s == null ? '' : ('' + s)
  starts = '' + starts
  position = position < 0 ? 0 : (+position || 0)
  position = position == null ? 0 : Math.min(position, s.length)
  return s.lastIndexOf(starts, position) === position
}

exports.endsWith = function(s, pattern) {
    var d = s.length - pattern.length
    return d >= 0 && s.indexOf(pattern, d) === d
}
